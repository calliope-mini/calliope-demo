...
void BluetoothServiceDebug::onDataRead(GattReadAuthCallbackParams *params)
{
    if (params->handle == characteristicsHandle) {
        ...
        if (selector == 0x00) {
            ...
            const uint8_t *buffer = (uint8_t*)&interpreter.code[0];

            const uint16_t start = MIN(address, CODE_LEN);
            const uint16_t stop = MIN(address+16, CODE_LEN);
        ...
        if (selector == 0x01) {
            ...
            const uint16_t start = MIN(address, METHODS_COUNT*2);
            const uint16_t stop = MIN(address+2, METHODS_COUNT*2);
            const uint16_t len = stop - start;
